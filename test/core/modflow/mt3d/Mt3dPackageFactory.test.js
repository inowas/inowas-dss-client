...
test('Create Adv-Package from Object', () => {
    ...
    expect(Mt3dPackageFactory.fromData(p.toObject)).toBeInstanceOf(AdvPackage);
    expect(Mt3dPackageFactory.fromData(p.toObject).toObject).toEqual(p.toObject);
});

test('Create Btn-Package from Object', () => {
    const p = BtnPackage.fromDefault();
    expect(Mt3dPackageFactory.fromData(p.toObject)).toBeInstanceOf(BtnPackage);
    expect(Mt3dPackageFactory.fromData(p.toObject).toObject).toEqual(p.toObject);
});

test('Create Dsp-Package from Object', () => {
    const p = DspPackage.fromDefault();
    expect(Mt3dPackageFactory.fromData(p.toObject)).toBeInstanceOf(DspPackage);
    expect(Mt3dPackageFactory.fromData(p.toObject).toObject).toEqual(p.toObject);
});

test('Create Gcg-Package from Object', () => {
    const p = GcgPackage.fromDefault();
    expect(Mt3dPackageFactory.fromData(p.toObject)).toBeInstanceOf(GcgPackage);
    expect(Mt3dPackageFactory.fromData(p.toObject).toObject).toEqual(p.toObject);
});

test('Create Mt-Package from Object', () => {
    const p = MtPackage.fromDefault();
    expect(Mt3dPackageFactory.fromData(p.toObject)).toBeInstanceOf(MtPackage);
    expect(Mt3dPackageFactory.fromData(p.toObject).toObject).toEqual(p.toObject);
});

test('Create Ssm-Package from Object', () => {
    const p = SsmPackage.fromDefault();
    expect(Mt3dPackageFactory.fromData(p.toObject)).toBeInstanceOf(SsmPackage);
    expect(Mt3dPackageFactory.fromData(p.toObject).toObject).toEqual(p.toObject);
});
