...
export default function* addLayerFlow() {
    ...
    while (true) {
        ...
        while (true) {
            // eslint-disable-next-line no-shadow
            const response = yield take( action => WebData.Helpers.waitForResponse( action, Command.ADD_LAYER ) );
            ...
            if (response.webData.type === 'success') {
                ...
                yield put( push(editLayerUrl( action.tool, action.payload.id, action.payload.layer.id ) ));
                break;
            }
        }
    }
