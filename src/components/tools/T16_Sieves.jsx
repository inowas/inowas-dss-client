...
export default class Sieves extends React.Component {
    ...
    subtraction( ) {
        const value = this.props.dSampSieve.value;
        const total = this.props.info.total;
    ...
    renderNumber( param ) {
        ...
        return (
            <tr key={param.id} className="parameter">
                ...
                <td>
                    <input name={'sieve_' + param.id + '_value'} className="parameter-max input input-xs" type="number" min={param.min} max={param.max} step={param.stepSize} value={Number( param.value ).toFixed( param.decimals )} onChange={this.handleChange}/>
                </td>
                <td>g</td>
            </tr>
        );
    ...
    selectSieve( ) {
        ...
        if ( this.props.DIN.selected === true ) {
            var standard = this.props.DIN.sievesize.map(s => {
                return (
                    <li key={'sievesize_' + s}>
                        <label><input name={'size ' + s} className="input" type="checkbox" onChange={this.handleChange}/>{s}</label>
                    </li>
                );
            });
        }
                ...
                <ul className="popup-ul">
                    <li>
                        <label className="label">
                            <input onChange={this.handleChange} className="input" name="standard" value={'standard_' + this.props.DIN.name} type="radio"/>DIN 18123:2010-03</label>
                        <label className="label">
                            <input onChange={this.handleChange} className="input" name="standard" value={'standard_' + this.props.ASTM.name} type="radio"/>ASTM D6913</label>
                    </li>
                </ul>
                <div className="popup-col">
                    <ul className="ul left">{standard}</ul>
                </div>
            </div>
        );
    }
            ...
            <div className="grid-container">
                <div className="col stretch parameters-wrapper">
                        ...
                        <tbody>
                            <tr key={dSampSieve.id} className="parameter">
                                <td className="parameter-label">
                                    <strong>{dSampSieve.name}</strong>
                                </td>
                                <td>
                                    <input name={'sieve_' + dSampSieve.id + '_value'} className="parameter-max input input-xs" type="number" min={dSampSieve.min} max={dSampSieve.max} step={dSampSieve.stepSize} value={Number( dSampSieve.value ).toFixed( dSampSieve.decimals )} onChange={this.handleChange}/>
                                </td>
                                <td>g</td>
                            </tr>
                        </tbody>
                    ...
                    <table className="parameters-sieve">
                        <tbody>
                            <tr>
                                <th className="table-sieve-bottom">
                                    <strong>Sieve</strong>
                                </th>
                                <th className="table-sieve-bottom">
                                    <strong>Mass</strong>
